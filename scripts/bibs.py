...
for pID, rec in sorted_data:
    # print rec
    try:
        s = "<strong class=\"ptitle\">%s</strong><br/>" % enc(rec[u"title"][0])
        if u"author" in rec:
        ...
        if u"booktitle" in rec:
            ...
            s += "%s, %s." % (journal, year)
            idx_add(journal, pID)
        papers[pID] = "<p class=\"paper\">%s <small><a href=\"http://dblp.uni-trier.de/rec/bibtex/%s\">bibtex</a></small></p>" % (s, enc(pID))

    except:
        print rec
        # raise

file("../data/index.json","w").write(json.dumps({"papers": papers, "index": index }))